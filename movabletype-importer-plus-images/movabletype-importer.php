<?php
/*
Plugin Name: Movable Type and TypePad Importer + Image Downloader
Plugin URI: http://brian2000.com
Description: Import posts and comments from a Movable Type or TypePad blog.
Author: Brian Kennedy
Author URI: http://brian2000.com
Version: 0.1
Stable tag: 0.1
Date of modification: 05/16/2013
...
Original Plugin URI: http://wordpress.org/extend/plugins/movabletype-importer/
Original Author: wordpressdotorg
Original Author URI: http://wordpress.org/
Original Plugin Version: 0.4
...
<p><?php _e('If a new user is created by WordPress, a password will be randomly generated. Manually change the user&#8217;s details if necessary.', 'movabletype-importer'); ?></p>
	...
	function get_images_from_line($line, $post_id){
		...
		foreach($html->find('img') as $e){
			if($e->src != ''){
				set_time_limit(30); //keeps script from timing out 
				$imageurl = $e->src;	
				$newfilename = explode('/',$e->src);
				$newfilename = end($newfilename).'.jpg';
	
				$uploads = wp_upload_dir();
				$filename = wp_unique_filename( $uploads['url'], $newfilename, $unique_filename_callback = null );
	
				$e->src =  $uploads['url'].'/'.$filename;
				}
		...
		foreach($html->find('a[class=asset-img-link]') as $e){
			if($e->href!= ''){				
				set_time_limit(300); //keeps script from timing out 
				$imageurl = $e->href;	
				$newfilename = explode('/',$e->href);
				$newfilename = end($newfilename).'.jpg';
	
				$uploads = wp_upload_dir();
				$filename = wp_unique_filename( $uploads['url'], $newfilename, $unique_filename_callback = null );
	
				$e->href =  $uploads['url'].'/'.$filename;
				}
				$image_string = $this->fetch_image($imageurl);
				$fileSaved = file_put_contents($uploads['path'] . "/" . $filename, $image_string);
				if ( !$fileSaved ) {
	...
	function media_process($imageurl, $post_id) {
			$uploads = wp_upload_dir();
	...
	function process_posts() {
		...
		while ( $line = $this->fgets($handle) ) {
			...
			if ( '-----' == $line ) {
				...
				if ( 'body' == $context ) {
					$post->post_content .= $this->get_images_from_line($line, $post->post_id );
				} else if ( 'extended' ==  $context ) {
					$post->extended .= $this->get_images_from_line($line, $post->post_id );
				} else if ( 'excerpt' == $context ) {
					$post->post_excerpt .= $this->get_images_from_line($line, $post->post_id );
